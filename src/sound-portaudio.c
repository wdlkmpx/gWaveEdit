...
{     
    ...
    if (XOR(format->bigendian,IS_BIGENDIAN)) {
        return 0;
    }
...
static gboolean portaudio_output_supports_format(Dataformat *format)
{
    ...
    return (i && format->channels <= i->maxOutputChannels && 
            ...
            sportaudio_samplesize_supported(i,format));
}


static gboolean portaudio_output_stop(gboolean must_flush)
{
    if (!portaudio_data.outstream) {
        return TRUE;
    }
    if (must_flush)
    {
                                     ...
                                     PaTimestamp outTime, void *userData)
{
    ...
    if (j>0) {
        memset(((gchar *)outputBuffer)+i, 0, j);
    }
...
{
    PaError err;
    if (!portaudio_output_supports_format(format)) {
        return -1;
    }
...
static guint portaudio_output_play(gchar *buffer, guint bufsize)
{
    ...
    PaTimestamp p;
    if (!bufsize) {
        return ringbuf_available(portaudio_data.output_buffer);
    }
...
static gint portaudio_input_select_format(Dataformat *format, gboolean silent)
{
    ...
    if (!portaudio_input_supports_format(format)) {
        return -1;
    }
