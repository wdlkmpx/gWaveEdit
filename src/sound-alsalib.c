                ...
                gboolean silent)
{
    ...
    if (*handp == NULL) {
        ...
        i = snd_pcm_open(handp, devname, str, SND_PCM_NONBLOCK);
        if (i<0) {
            if (!silent) {
                c = g_strdup_printf((str == SND_PCM_STREAM_CAPTURE)?
                    _("Error opening ALSA device '%s' for recording: %s"):
                    _("Error opening ALSA device '%s' for playback: %s"),
                    devname, snd_strerror(i));
                user_error(c);
                g_free(c);
            }
            return TRUE;
...
static gboolean alsa_set_format(
                Dataformat *format,
                Dataformat *fmtp,
                snd_pcm_t **handp,
                gboolean playback,
...
{     
    ...
    struct pollfd *fds;
    GPollFD *pfds;
    if (dataformat_equal(fmtp,format)) {
        return FALSE;
    }
    snd_pcm_hw_params_malloc (&par);
    snd_pcm_hw_params_any (*handp,par);

    i = snd_pcm_hw_params_set_access (*handp, par, SND_PCM_ACCESS_RW_INTERLEAVED);
    if (i) {
    ...
    alsa_data.iogroup = mainloop_io_group_add(
                        fd_count,
                        pfds,
                        wdtime/1000,
...
static guint alsa_output_play(gchar *buffer, guint bufsize)
{
    ...
    if (!alsa_data.inside_ready_func) {
        ...
        printf("called output_play from ready-func, %d bytes, time=%3d.%06d\n",
            (int)bufsize,(int)tv.tv_sec,(int)tv.tv_usec);
    }
#endif
...
static void alsa_input_store(Ringbuf *buffer)
{
    ...
    while (1) {
        x = snd_pcm_readi(
                alsa_data.rhand,
                buf,
                MIN (sizeof(buf),ringbuf_freespace(buffer))/alsa_data.rfmt.samplebytes
        );
        if (x > 0) {
