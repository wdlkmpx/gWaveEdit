...
static struct sound_driver drivers[] = {
...
#ifdef HAVE_ALSALIB
	{
		...
		alsa_input_store,
		alsa_input_stop,
		alsa_input_stop_hint,
		alsa_input_overrun_count
	},
#endif
...
#ifdef HAVE_JACK
	{
		...
		mhjack_output_play,
		mhjack_output_stop, 
		mhjack_clear_buffers,
		mhjack_output_suggest_format,
		NULL,
		mhjack_input_supported_formats,  
		mhjack_input_select_format,
		mhjack_input_store,
		mhjack_input_stop,
		mhjack_input_stop,
		mhjack_get_xrun_count
	},
#endif
...
#ifdef HAVE_SUN
	{
		...
		sunaud_init,
		sunaud_quit,
		sunaud_output_select_format,
		sunaud_output_want_data,
		sunaud_output_play,
		sunaud_output_stop,
		...
		NULL,
		input_supported_true,
		sunaud_input_select_format,
		sunaud_input_store,
		sunaud_input_stop,
		NULL,
		sunaud_input_overrun_count
	},
#endif
...
#if defined (HAVE_PORTAUDIO)
	{
		...
		portaudio_output_want_data,
		portaudio_output_play,
		portaudio_output_stop,
		portaudio_output_clear_buffers,
		NULL,
		NULL,
		portaudio_input_supported_formats,
		portaudio_input_select_format,
		portaudio_input_store,
		portaudio_input_stop,
		NULL,
		portaudio_input_overrun_count
	},
#endif
...
#if defined (HAVE_SDL)
	{
		...
		sdl_output_want_data,
		sdl_output_play,
		sdl_output_stop,
		sdl_output_clear_buffers,
		NULL,
		NULL,
		NULL,
		NULL,
...
#ifdef HAVE_PULSEAUDIO
	{
		...
		pulse_init,
		pulse_quit,
		pulse_output_select_format,
		pulse_output_want_data,
		pulse_output_play,
		pulse_output_stop,
		pulse_output_clear_buffers,
		NULL, 
		pulse_needs_polling, 
		pulse_input_supported_formats,
		pulse_input_select_format,
		pulse_input_store,
		pulse_input_stop,
		NULL,
...

	{
		...
		dummy_output_want_data,
		dummy_output_play,
		dummy_output_stop,
		dummy_output_clear_buffers,
		NULL,
		NULL,
		dummy_input_supported_formats,
		dummy_input_select_format,
		dummy_input_store,
		dummy_input_stop
	}
...
gboolean sound_driver_has_preferences(gchar *id)
{
	...
	if (!id) {
		return (drivers[current_driver].preferences != NULL);
	}
...
gboolean sound_poll(void)
{
	...
	if (!output_ready_func && !input_ready_func) {
		return -1;
	}
...
static void sound_output_ready_func(void)
{
	...
	if (sound_delayed_quit) {
		while (output_want_data()) {
			output_play(zerobuf,
				sizeof(zerobuf) - (sizeof(zerobuf)%playing_format.samplebytes));
		}
